...
def connect(db):
    '''
    连接数据库
    :param db: 字典式的数据 # db = {"ip": "jy001", "port": "4406", "user": "root", "pwd": "123456", "dbname": "future"}
    :return: 连接对象
    '''
    # 从字典里取出数据库相关的值
    ip = db['ip']
...
#     conn = connect(db)
#     # 根据手机号删除用户
#     excute(conn, "delete from member where mobilephone=18091234567;")
#     disconnect(conn)
